...
class CustomEnv(gym.Env):
    def __init__(self, port: int):
        ...
        - Vector formed by step t and t+1 trajectory points : X [16], Y[17], Z [18]
        """
    ...
    def calculate_reward(self, obs, done):
        ...
        eud = obs[15]
        # rw_dist = 10.0 - (gain_pos*eud)
        # rw_dist = np.clip(rw_dist, 0.0, 10.0)
        ...
        # rw_progress = 0.0
        # if self.prev_eud is not None:
        #     delta = self.prev_eud - eud
        #     rw_progress = 20.0 * delta  # positive if it improves negative otherwise
        #     rw_progress = np.clip(rw_progress, -2.0, 2.0)
...
def main():
    ...
    # model = SAC.load("/home/oscar/panda_myoarm/checkpoints/sac_panda_home_v25_1950000_steps.zip")
    # model.load_replay_buffer("/home/oscar/panda_myoarm/checkpoints/sac_panda_home_v25_latest_replay_buffer.pkl")
    # model.set_env(env)

    name_prefix = "sac_panda_home_v31"
    # Train the model
