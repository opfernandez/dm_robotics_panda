...
# class MyoArm(prop.Prop):
#   """Prop defined by an external XML file."""
  
#   def _build(self, xml_path: str, name: str, *args, **kwargs):
#       del args, kwargs
#       # Carga el modelo desde el XML
#       mjcf_model = mjcf.from_path(xml_path)
#       super()._build(name, mjcf_model)

class MyoArm(Entity):
  """An entity class that wraps an MJCF model without any additional logic."""

  def _build(self, xml_path):
    self.mjcf_model = mjcf.from_path(xml_path)
...
if __name__ == '__main__':
  ...
  XML_ARM_PATH = "/home/oscar/TFM/models/myo_sim/arm/brick.xml"
  ...
  # Carga el prop desde el XML
  # xml_prop = MyoArm(xml_path="/home/oscar/TFM/models/myo_sim/arm/brick.xml", name="myoarm")

  # # Añade solo ese prop a la lista
  # props = [xml_prop]

  # # Añade los props al entorno
