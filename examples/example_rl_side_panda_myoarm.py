...
class CustomEnv(gym.Env):
    def __init__(self, port: int):
        ...
        """
        self.observation_space = Box(low=-np.inf, high=np.inf, shape=(X,), dtype=np.float32)
        # Define the action space limits
        # Replace X with the actual number of actions or dimensions
        low_ap_limits = np.array([X,X,X, ...], dtype=np.float32)
        up_ap_limits = np.array([X,X,X, ...], dtype=np.float32)
        self.action_space = Box(low=low_ap_limits, high=up_ap_limits, shape=(X,), dtype=np.float32)
    ...
    def format_actions(self, actions):
        ...
        """
        # Format actions to be sent to the agent side
        act = {}
        return act
