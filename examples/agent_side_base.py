...
class BaseAgent(ABC):
    ...
    This class serves as a base class for specific agent implementations.
    step() and reset() methods must be implemented by child classes.
    """
    ...
    def calculate_trajectory(self, ef_position, uc="h-square"):
        ...
        #######################################################
            ...
            # Pentagon parameters
            r = 0.07  # Circumscribed circle radius
            num_sides = 5
            points_per_side = 24  # 120 total points / 5 sides
            total_vertices = []
            center_x, center_y = posX, posY

            # Calculate the vertices of the pentagon (top vertex at 12 o'clock)
            for i in range(num_sides):
                angle = 2 * np.pi * i / num_sides + np.pi / 2  # top vertex
                x = center_x + r * np.cos(angle)
                y = center_y + r * np.sin(angle)
                total_vertices.append((x, y))
            total_vertices.append(total_vertices[0])  # close the figure
            # Create the trajectory with 24 points per side
            trajectory = []
            laps = n_points // 120
            if (n_points % 120) != 0:
                laps += 1
            ...
            self.trajectory = np.array(trajectory, dtype=np.float32)
        #######################################################
        elif uc == "h-pentagon":
            # Pentagon parameters
            r = 0.07  # Circumscribed circle radius
    ...
    def step(self, timestep: dm_env.TimeStep) -> np.ndarray:
        ...
        It communicates with the stable-baselines agent to receive the action to be executed 
...
def init_environment() -> tuple[Environment, list]:
    """Initialize the simulation environment.
    Returns:
        env: The initialized environment.
        joint_names: The list of joint names of the robot in the environment.
    """
